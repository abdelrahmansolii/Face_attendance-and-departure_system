...
class Face_Register:
        ...
        if self.face_folder_created_flag:
            if self.current_frame_faces_cnt == 1:
                if not self.out_of_range_flag:
                    ...
                    self.face_ROI_image = np.zeros((int(self.face_ROI_height), self.face_ROI_width, 3),
                                                   np.uint8)
                    for ii in range(self.face_ROI_height):
                        for jj in range(self.face_ROI_width):
                            self.face_ROI_image[ii][jj] = self.current_frame[self.face_ROI_height_start + ii][
                                self.face_ROI_width_start + jj]
    ...
    def get_frame(self):
        try:
            if self.cap.isOpened():
                ...
                logging.error("Webcam is not opened.")
                return False, None
    ...
    def process(self):
        ...
        if not ret or self.current_frame is None:  # Check if frame capture failed
            logging.error("No frame captured from webcam.")
            self.win.after(20, self.process)  # Continue trying to process frames
            return
        ...
        if ret:
            ...
            if len(faces) != 0:
                ...
                for k, d in enumerate(faces):
                    ...
                    # Draw rectangle around the face without extra padding
